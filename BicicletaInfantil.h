...
public:
	...
	BicicletaInfantil(const BicicletaInfantil& bi): Bicicleta(bi) { m_plegable = plegable; }
	BicicletaInfantil* clone() { return new BicicletaInfantil(this*); }
	~BicicletaInfantil() {}

	bool getPlegable() const { return m_plegable; }
	void setPlegable(const bool& plegable) { if(plegable) m_plegable =  true; else m_plegable = false; }
